...
new #[Title('Create Server')] class extends Component
{
    ...
    public function updatedPort(int $value): void
    {
        $this->query_port = $value + 1;
    }
    ...
    public function save(): void
    {
        ...
        Server::query()->create($validated);

        session()->flash('status', "Server '{$validated['name']}' created successfully.");
...
<section class="w-full">
    <div class="mb-6">
        <flux:heading size="xl">{{ __('Create Server') }}</flux:heading>
        <flux:text class="mt-2">{{ __('Configure a new Arma 3 server instance.') }}</flux:text>
    </div>
